...
# file paths
root = '/opt/sdb/workspace/data/fire-data'
images_dir = os.path.join(root, './JPEGImages')
annotations_dirs = os.path.join(root, './Annotations')
train_image_set_filename = os.path.join(root, './ImageSets/train.txt')
val_image_set_filename = os.path.join(root, './ImageSets/val.txt')
...
train_dataset.parse_xml(images_dirs=[images_dir], image_set_filenames = [train_image_set_filename],
                        annotations_dirs=[annotations_dirs], include_classes='all', ret=False)

val_dataset.parse_xml(images_dirs=[images_dir], image_set_filenames = [val_image_set_filename],
                       annotations_dirs=[annotations_dirs], include_classes='all', ret=False)
...
model_checkpoint = ModelCheckpoint(
                 os.path.join(log_dir, "ssdseg_firedata_{epoch:02d}_loss-{loss:.4f}_val_loss-{val_loss:.4f}.h5"),
                 monitor='val_loss', verbose=1, save_best_only=True, save_weights_only=True)
early_stopping = EarlyStopping(monitor='val_loss', min_delta=0, patience=10, verbose=1)

callbacks = [lr_scheduler, tensorboard, early_stopping, model_checkpoint]

model.fit_generator(train_generator, epochs=200, steps_per_epoch=200,
                    callbacks=callbacks, validation_data=val_generator,
                    validation_steps=50, initial_epoch=0)
